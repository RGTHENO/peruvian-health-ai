...
let cachedEncountersRaw = "__initial__";
let cachedEncountersSnapshot: Encounter[] = mockEncounters;

const canUseStorage = () => typeof window !== "undefined" && typeof window.localStorage !== "undefined";

const parseStoredEncounters = (stored: string | null): Encounter[] => {
  try {
...
const readStoredEncounters = (): Encounter[] => {
  ...
  return parseStoredEncounters(window.localStorage.getItem(ENCOUNTERS_STORAGE_KEY));
};
...
export const getEncounters = (): Encounter[] => {
  ...
  if (rawEncounters === cachedEncountersRaw) {
    return cachedEncountersSnapshot;
  }

  cachedEncountersRaw = rawEncounters;
  cachedEncountersSnapshot = parseStoredEncounters(rawEncounters);
